...
use crate::managers::qwen3asr::{init_qwen3asr_python_path, Qwen3ASR, Qwen3InferenceParams};
use crate::settings::{
...
impl TranscriptionManager {
    ...
    pub fn unload_model(&self) -> Result<()> {
...

        {
            ...
            if let Some(LoadedEngine::Qwen3(ref mut qwen3asr)) = *engine {
                qwen3asr.unload_model();
            }
    ...
    pub fn load_model(&self, model_id: &str) -> Result<()> {
        ...
        let loaded_engine = match model_info.engine_type {
            ...
            EngineType::Qwen3 => {
                self.model_manager
                    .ensure_qwen3asr_python_runtime_ready(None)
                    .map_err(|e| {
                        let error_msg = format!("Failed to prepare Qwen3 Python runtime: {}", e);
                        emit_loading_failed(&error_msg);
                        anyhow::anyhow!(error_msg)
                    })?;
                init_qwen3asr_python_path(&self.app_handle).map_err(|e| {
