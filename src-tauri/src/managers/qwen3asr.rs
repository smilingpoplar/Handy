...
static QWEN3ASR_PYTHON_PATH: OnceLock<String> = OnceLock::new();
static QWEN3ASR_SCRIPT_PATH: OnceLock<String> = OnceLock::new();
pub(crate) const QWEN3ASR_DEFAULT_ENDPOINT: &str = "https://hf-mirror.com";

pub(crate) fn init_qwen3asr_python_path(
    app_handle: &AppHandle,
) -> std::result::Result<(), Box<dyn std::error::Error>> {
    let app_data_dir = crate::portable::app_data_dir(app_handle)?;
    let runtime_dir = app_data_dir.join("qwen3asr_mlx");
    let python_path = runtime_dir.join(".venv/bin/python3");
    let _ = QWEN3ASR_PYTHON_PATH.set(python_path.to_string_lossy().to_string());
    let script_path = runtime_dir.join("qwen3asr_server.py");
    if script_path.exists() {
        let _ = QWEN3ASR_SCRIPT_PATH.set(script_path.to_string_lossy().to_string());
    }
    Ok(())
}

fn resolve_server_script_path() -> std::result::Result<PathBuf, Box<dyn std::error::Error>> {
    if let Some(cached) = QWEN3ASR_SCRIPT_PATH.get() {
    ...
    let python_path = QWEN3ASR_PYTHON_PATH.get().ok_or_else(|| {
        std::io::Error::new(
...
fn resolve_python_command() -> std::result::Result<String, Box<dyn std::error::Error>> {
    let fixed_python = QWEN3ASR_PYTHON_PATH.get().ok_or_else(|| {
        std::io::Error::new(
            std::io::ErrorKind::Other,
            "Qwen3 Python path is not initialized",
        )
...
pub(crate) fn get_qwen3asr_python_command(
...
fn run_qwen3asr_model_info(
    python_cmd: &str,
    python_args: &[String],
    repo_id: &str,
) -> anyhow::Result<Qwen3ModelInfo> {
    let python_path = QWEN3ASR_PYTHON_PATH
        .get()
    ...
    cmd.env("HF_ENDPOINT", QWEN3ASR_DEFAULT_ENDPOINT);
...
pub(crate) fn resolve_qwen3asr_model_info(repo_id: &str) -> anyhow::Result<Qwen3ModelInfo> {
    let (python_cmd, python_args) = get_qwen3asr_python_command()
        .map_err(|e| anyhow::anyhow!("Failed to resolve Python for Qwen3 model info: {}", e))?;
    let model_info = run_qwen3asr_model_info(&python_cmd, &python_args, repo_id)?;
...
pub(crate) fn build_qwen3asr_file_url(
    endpoint: &str,
    repo_id: &str,
    revision: &str,
    filename: &str,
